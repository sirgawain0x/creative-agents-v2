...
describe("getAgent1Policy slice C", () => {
  beforeEach(() => {
  ...
  it("defaults to disabled dry-run gates", () => {
    ...
    expect(policy.version).toBe("slice-c");
  ...
  it("uses dry_run mode when trading enabled without kill switch", () => {
    ...
    expect(policy.trading.reason).toBe("trading_enabled_but_slice_c_dry_run_only");
  });
  ...
  it("executeTradingNoOp never executes", () => {
    ...
    expect(result.reason).toBe("slice_c_dry_run_only");
  });
});
