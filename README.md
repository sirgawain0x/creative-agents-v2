...
Deployable Next.js App Router scaffold for Creative Agents (`creative-agents` package). **Slice E prep** adds durable tick state (Upstash Redis) and a G2-gated MeTokens venue confirm path — **no live trading or broadcast**.

## Survival First

- **Trading is disabled by default** (`TRADING_ENABLED=false`).
- Slice E prep may plan/prepare calls via the dry-run path; it never calls `wallet_sendPreparedCalls` / `wallet_sendCalls`.
...
| --- | --- | --- |
| `TRADING_ENABLED` | `false` | Master gate (tick still only dry-runs when true) |
...
| `AGENT1_TICK_MODEL_ENABLED` | `false` | Optional model path (stub; Slice E live blocked) |
...
| `GET /api/agent1/venue` | Read-only venue status (addresses, ABI label, `routerConfirmed`, quote mode) |
...
| `GET/POST /api/agent1/dry-run` | Dry-run: quote + planned calls + optional Alchemy prepare |
| `GET/POST /api/agent1/tick` | Cron tick loop: policy → candidate selection → dry-run plan |
| `GET /api/agent2/health` | Agent 2 stub (future slice) |

### Slice E prep — durable tick state

Daily USDC volume (UTC day key) and last-planned cooldown timestamp persist via **Upstash Redis REST** when `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` are set. Without them, the app uses an in-memory store (fine for local dev; **not** durable across Vercel cold starts).

If the store errors on read/write, the tick **fail-closes** (`decision.reason: tick_store_error`) — no plan is recorded and volume is not invented.

### Slice E prep — venue confirm (no broadcast)
...
2. Set `CRON_SECRET`, `UPSTASH_REDIS_REST_*`, wallet/RPC env as needed.
...
| **E prep** (current) | Upstash tick store + venue confirm gate + `/api/agent1/venue` | G2 `AGENT1_ROUTER_CONFIRMED`; Upstash in Vercel |
| **E live** | Live execute + Gemini + cron broadcast | Vault funding, explicit broadcast enablement, model keys |
